...
async def handle_webapp(message: Message):
    try:
        ...
        if sheet:
            try:
                sheet.append_row([oid, datetime.now().strftime('%d.%m %H:%M'), data['name'], data['phone'], f"{cat}{w_str}: {data['what']}", f"{data['price']} UZS", "🆕"])
            except: pass
        ...
        text_adm = (f"🚚 <b>ЗАКАЗ #{oid}</b>\n━━━━━━━━━━━━━━━\n"
                    ...
                    f"👤 <b>Имя:</b> {data['name']}\n📞 <b>Тел:</b> {data['phone']}\n\n"
                    f"📍 <a href='{url_a}'>ОТКУДА</a>\n🏁 <a href='{url_b}'>КУДА</a>")

        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚕 ПРИНЯТЬ ЗАКАЗ", callback_data=f"acc_{oid}_{message.from_user.id}_{lang}")]])
        for aid in ADMIN_IDS:
            await bot.send_message(aid, text_adm, reply_markup=kb, parse_mode="HTML", disable_web_page_preview=True)

        resp = f"✅ <b>Заказ №{oid} оформлен!</b>" if lang == 'ru' else f"✅ <b>Buyurtma №{oid} qabul qilindi!</b>"
        await message.answer(resp, parse_mode="HTML")
    except Exception as e: print(f"WEBAPP ERROR: {e}")
...
async def accept(callback: CallbackQuery):
    ...
    kb_done = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🏁 ДОСТАВЛЕНО", callback_data=f"done_{oid}_{uid}_{lang}")]])
    await callback.message.edit_text(callback.message.html_text + f"\n\n🤝 <b>Взял: {callback.from_user.first_name}</b>", reply_markup=kb_done, parse_mode="HTML", disable_web_page_preview=True)
    msg = "🚕 Курьер принял заказ!" if lang == 'ru' else "🚕 Kuryer buyurtmani qabul qildi!"
...
async def done(callback: CallbackQuery):
    ...
    await callback.message.edit_text(callback.message.html_text + "\n\n✅ <b>ДОСТАВЛЕНО</b>", reply_markup=None, parse_mode="HTML")
    msg = "🏁 Доставлено!" if lang == 'ru' else "🏁 Yetkazildi!"
    await bot.send_message(uid, msg)
